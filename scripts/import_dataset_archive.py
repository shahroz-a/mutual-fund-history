...
The archive is expected to contain latest.csv.gz and a Year/ directory, either at
...
def find_payload(root: Path) -> Path:
    for candidate in candidate_roots(root):
        if (candidate / REQUIRED_LATEST).is_file() and (candidate / REQUIRED_TREE).is_dir():
            return candidate
    raise RuntimeError("Archive must contain latest.csv.gz and Year/.")
...
def import_payload(payload: Path, data_dir: Path) -> None:
    ...
    copied = 0
    for source_file in sorted(source_tree.rglob("*.csv.gz")):
